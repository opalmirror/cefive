 ...
 * PSP_Lord: These type defintions are declared in stdint.h, but for whatever reason,
 * including this header file won't solve the problem of 
 * "Unable to reolve identifier of uint32_t".  
 */
...
struct BitmapHeader
{
  ...
  uint32_t reserved; //6
  uint32_t offset; //10
  uint32_t headsize; //14
  uint32_t width; //18
  uint32_t height; //22
  uint16_t planes; //26
  uint16_t bpp; //30
  uint32_t comp; //34
  uint32_t bitmapsize; //38
  uint32_t hres; //42
  uint32_t vres; //46
  uint32_t colors; //50
  uint32_t impcolors; //54
...
int write_5551_data(void *frame, int pixel_data)
{
  ...
  for(h = 271; h >= 0; h--)
  {
    ...
    sceIoLseek(screenshotFd, line, SEEK_SET); 
    sceIoWrite(screenshotFd, &fileBuffer, 480*3);
    line += 480*3;
  }
